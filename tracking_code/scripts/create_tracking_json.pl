...
foreach my $line_name (@{$hESCreg->find_lines()}) {
  ...
  ${$line_output}->{hESCreg} = {
    name => $line->{name},
    exported => 1,
    validated => $line->{validation_status} == 1 ? 1 : 0,
...
foreach my $line_hash (@{$output{lines}}) {
  ...
  if (my $donor_biosample_id = $line_hash->{hESCreg}{donor_biosample_id} || $line_hash->{IMS}{donor_biosample_id}) {
    if (BioSD::fetch_sample($donor_biosample_id)) {
      $line_hash->{donor_biosample} = {exported =>1, id => $donor_biosample_id};
    }
    else {
      $line_hash->{donor_biosample} = {exported =>0, id => $donor_biosample_id};
    }
  }

  my %tests = (
    'IMS exports the cell line' => $line_hash->{IMS}{exported} ? 'pass' : 'fail',
    'hPSCreg exports the cell line' => $line_hash->{hESCreg}{exported} ? 'pass' : 'fail',
    'BioSamples exports the cell line' => $line_hash->{biosample}{exported} ? 'pass' : 'fail',
    'IMS exports a cell line name (where line is exported)' => !$line_hash->{IMS}{exported} ? 'cannot test'
                                  : !$line_hash->{IMS}{name} ? 'fail' : 'pass',
    'IMS exports a biosample ID (where line is exported)' => !$line_hash->{IMS}{exported} ? 'cannot test'
                                  : !$line_hash->{IMS}{biosample_id} ? 'fail' : 'pass',
    'IMS exports a donor biosample ID (where line is exported)' => !$line_hash->{IMS}{exported} ? 'cannot test'
                                  : !$line_hash->{IMS}{donor_biosample_id} ? 'fail' : 'pass',
    'hPSCreg exports a cell line name (where line is exported)' => !$line_hash->{hESCreg}{exported} ? 'cannot test'
                                  : !$line_hash->{hESCreg}{name} ? 'fail' : 'pass',
    'hPSCreg exports a biosample ID (where line is exported)' => !$line_hash->{hESCreg}{exported} ? 'cannot test'
                                  : !$line_hash->{hESCreg}{biosample_id} ? 'fail' : 'pass',
    'hPSCreg exports a donor biosample ID (where line is exported)' => !$line_hash->{hESCreg}{exported} ? 'cannot test'
                                  : !$line_hash->{hESCreg}{donor_biosample_id} ? 'fail' : 'pass',
    'name in IMS consistent with name in hPSCreg (where both have a name)' => !$line_hash->{IMS}{exported}  || !$line_hash->{IMS}{name} ? 'cannot test'
                                  : !$line_hash->{hESCreg}{exported}  || !$line_hash->{hESCreg}{name} ? 'cannot test'
                                  : $line_hash->{IMS}{name} eq $line_hash->{hESCreg}{name} ? 'pass' : 'fail',
    'biosample id in IMS consistent with BioSamples database (where both have a biosample id)' => !$line_hash->{IMS}{exported}  || !$line_hash->{IMS}{biosample_id} ? 'cannot test'
                                  : !$line_hash->{biosample}{exported}  || !$line_hash->{biosample}{id} ? 'cannot test'
                                  : $line_hash->{biosample}{id} eq $line_hash->{IMS}{biosample_id} ? 'pass' : 'fail',
    'biosample id in hPSCreg consistent with BioSamples database (where both have an id)' => !$line_hash->{hESCreg}{exported}  || !$line_hash->{hESCreg}{biosample_id} ? 'cannot test'
                                  : !$line_hash->{biosample}{exported}  || !$line_hash->{biosample}{id} ? 'cannot test'
                                  : $line_hash->{biosample}{id} eq $line_hash->{hESCreg}{biosample_id} ? 'pass' : 'fail',
    'biosample id in IMS consistent with biosample id in hPSCreg (where both have an id)' => !$line_hash->{IMS}{exported}  || !$line_hash->{IMS}{biosample_id} ? 'cannot test'
                                  : !$line_hash->{hESCreg}{exported}  || !$line_hash->{hESCreg}{biosample_id} ? 'cannot test'
                                  : $line_hash->{hESCreg}{biosample_id} eq $line_hash->{IMS}{biosample_id} ? 'pass' : 'fail',
    'donor biosample id in IMS consistent with donor biosample id in hPSCreg (where both have an id)' => !$line_hash->{IMS}{exported}  || !$line_hash->{IMS}{donor_biosample_id} ? 'cannot test'
                                  : !$line_hash->{hESCreg}{exported}  || !$line_hash->{hESCreg}{donor_biosample_id} ? 'cannot test'
    ...
    'Biosamples exports the donor (donor biosample id is taken from hPSCreg or IMS)' => !$line_hash->{hESCreg}{donor_biosample_id}  && !$line_hash->{IMS}{donor_biosample_id} ? 'cannot test'
                                  : $line_hash->{donor_biosample}{exported} ? 'pass' : 'fail',
